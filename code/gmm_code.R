#####this file contains the variational approximation to gmm##
...
#####new add#######
# Fit VBLR model with stochatiscal approximation
vb_gmm_svi <- function(X, K = 3, alpha_0 = 1/K, m_0 = c(colMeans(X)), 
                       beta_0 = 1, nu_0 = NCOL(X) + 50, 
                       W_0 = diag(100, NCOL(X)), max_iter = 500, 
                       epsilon_conv = 1e-4, is_animation = FALSE, 
                       is_verbose = FALSE){
  ...
  for (i in 2:max_iter) {
    ...
    for (k in 1:K) {
      diff <- sweep(X, MARGIN = 2, STATS = m_k[, k], FUN = "-")
      log_rho_nk[, k] <- log_pi[k] + 0.5*log_Lambda[k] - 
        0.5*(D/beta_k[k]) - 0.5*10*nu_k[k] * diag(diff %*% 
                                                    W_k[,,k] %*% t(diff)) # log of 10.67
    }
    # Responsibilities using the logSumExp for numerical stability
    Z        <- apply(log_rho_nk, 1, log_sum_exp)
    ...
    # Variational M-Step
    ##-------------------------------
    N_k <- 10*colSums(r_nk) + 1e-10  # 10.51
    for (k in 1:K) {
      x_bar_k[, k] <- (r_nk[ ,k] %*% X) / N_k[k]   # 10.52
      x_cen        <- sweep(X,MARGIN = 2,STATS = x_bar_k[, k],FUN = "-")
      S_k[, , k]   <- t(x_cen) %*% (x_cen * r_nk[, k]) / N_k[k]  # 10.53
...
################simulation code##########################
gauss_mix_simul <- function(n, mu, sigma, pi) {
  K <- length(mu) # Number of components
  components <- sample(1:K, size=n, replace=TRUE, prob=pi) # Determine components for each sample
  m_samples <- matrix(nrow=n, ncol=length(mu[[1]])) # Initialize matrix for samples
  
  for (i in 1:n) {
    comp <- components[i] # Get component for the i-th sample
    m_samples[i,] <- rmvnorm(n=1, mean=mu[[comp]], sigma=sigma[[comp]]) # Generate sample from the selected component
  }
